...
class BasicDataset(object):
    ...
    def get_test_json(self, mini_set=False):
        ...
        if self.pre_test_result:
            ...
            pre_queries = None
            pre_sqls = None
        return self.data_pre_process(tests, linking_infos, pre_queries, pre_sqls)
            ...
            for line in f.readlines():
                if line.strip():
                    try:
                        ...
                        print("JSON parse error. Skip")
                        print(f"ERROR msg: {e.msg}")
                        continue
        print(f"Parse {cnt} lines successfully.")
        return linking_infos
    ...
    def data_pre_process(self, datas, linking_infos=None, pre_queries=None, pre_sqls=None):
        db_id_to_table_json = dict()
        for table_json in self.get_table_json():
            db_id_to_table_json[table_json["db_id"]] = table_json
        for data in datas:
            ...
            if data["query"].strip()[:6] != 'SELECT' and data["query"].strip()[:6] != 'select':
                data["query_skeleton"] = data["query"]
                data["masked_sql"] = data["query"]
            else:
                data["query_skeleton"], data["masked_sql"] = sql2skeleton(data["query"], db_id_to_table_json[db_id])
            data["path_db"] = self.get_path_db(db_id)
        if pre_sqls:
            print("----------------------------------start process pre_sqls")
            for data, pre_sql in zip(datas, pre_sqls):
                data["pre_sql"] = pre_sql
                if pre_sql.strip()[:6] != 'SELECT' and data["query"].strip()[:6] != 'select':
                    data["masked_pre_sql"] = pre_sql
                else:
                    _, data["masked_pre_sql"] = sql2skeleton(pre_sql, db_id_to_table_json[db_id])
            for idx, data in enumerate(datas):
                if "masked_pre_sql" not in data:
                    print(f"{idx} pre sql not exist!")
        if linking_infos:
            db_id_to_table_json = dict()
            for table_json in self.get_table_json():
                db_id_to_table_json[table_json["db_id"]] = table_json
...
class EhrsqlDataset(BasicDataset):
    name = "ehrsql"
    test_json = "dev.json"
    test_gold = "dev.sql"
    train_json = "train.json"
...
def load_data(data_type, path_data, pre_test_result=None):
    ...
    elif data_type.lower() == "ehrsql":
        return EhrsqlDataset(path_data, pre_test_result)
    elif data_type.lower() == "spider-dk":
        return DKDataset(path_data, pre_test_result)
    elif data_type.lower() == "spider-realistic":
        return RealisticDataset(path_data, pre_test_result)
    elif data_type.lower() == "spider-syn":
        return SynDataset(path_data, pre_test_result)
    else:
        raise RuntimeError()
