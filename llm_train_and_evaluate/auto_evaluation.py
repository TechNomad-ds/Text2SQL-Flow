...
os.environ["NLTK_DATA"] = "/mnt/dataflow/cqf/nltk_data"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type = str, default = "./eval_output", help = "Base directory to save all outputs. Each eval_name will have its own subdirectory")
    ...
    # Create single output directory for this eval_name
    eval_output_dir = os.path.join(opt.output_dir, opt.eval_name)
    ...
    for ckpt_id in tqdm(ckpt_ids):
        ...
        if ckpt_id not in greedy_search_acc_dict.keys():
            # greedy decoding
            gs_pred_file = os.path.join(eval_output_dir, f"greedy_search_{ckpt_id}.json")
            greedy_search_cmd = f"CUDA_VISIBLE_DEVICES={opt.visible_devices} {python_cmd} infer.py \
            ...
            visualize(opt.eval_name, greedy_search_acc_dict, "greedy_search",
                os.path.join(eval_output_dir, "greedy_search.png"))
            save_evaluation_results(os.path.join(eval_output_dir, "greedy_search.json"), greedy_search_acc_dict)
        else:
            print(f"skip {ckpt_id} greedy search")

        if ckpt_id not in major_voting_acc_dict.keys():
            # sampling
            sampling_pred_file = os.path.join(eval_output_dir, f"sampling_{ckpt_id}.json")
            sampling_cmd = f"CUDA_VISIBLE_DEVICES={opt.visible_devices} {python_cmd} infer.py \
                --pretrained_model_name_or_path {os.path.join(opt.output_ckpt_dir, ckpt_id)} \
                --input_file {opt.input_file} \
            ...
            if opt.source in ["bird", "spider2.0"]:
                ...
                visualize(opt.eval_name, pass_at_k_acc_dict, "pass_at_k",
                    os.path.join(eval_output_dir, "pass_at_k.png"))
                save_evaluation_results(os.path.join(eval_output_dir, "pass_at_k.json"), pass_at_k_acc_dict)
            ...
            visualize(opt.eval_name, major_voting_acc_dict, "major_voting",
                os.path.join(eval_output_dir, "major_voting.png"))
            save_evaluation_results(os.path.join(eval_output_dir, "major_voting.json"), major_voting_acc_dict)
        else:
